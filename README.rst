...
این تابع برای ثبت _`سینک ها` استفاده می شود که آنها مسئول مدیریت _`پیام های واقعه ` به شکل _`گزارشی از نوع دیکشنری` هستند. یک سینک می تواند اشکال مختلفی داشته باشد: یک تابع ساده، یک رشته رشته، یک شی فایل مانند، یک تابع موازی (coroutine) یا یک نگه دارنده داخلی.
